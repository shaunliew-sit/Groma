#!/bin/bash
################################################################################
# SWIG-HOI Grounding Task Evaluation Script for Groma Qwen3VL
# Evaluates Groma Qwen3VL grounding performance on SWIG-HOI dataset
...
#
# Usage:
#   bash scripts/run_swig_ground_groma_qwen_eval.sh [GPU] [MODEL] [OUTPUT_DIR]
#
# Examples:
#   # Basic usage
#   bash scripts/run_swig_ground_groma_qwen_eval.sh 0                    # Use GPU 0 with defaults
#   bash scripts/run_swig_ground_groma_qwen_eval.sh 1                    # Use GPU 1 with defaults
#   bash scripts/run_swig_ground_groma_qwen_eval.sh 0,1                  # Use GPUs 0 and 1
#
#   # With optional flags (set as environment variables)
#   VERBOSE=1 bash scripts/run_swig_ground_groma_qwen_eval.sh 0          # Show per-image results + visualizations
#   MAX_IMAGES=10 bash scripts/run_swig_ground_groma_qwen_eval.sh 0      # Test on first 10 images
#   VERBOSE=1 MAX_IMAGES=10 bash scripts/run_swig_ground_groma_qwen_eval.sh 1  # Both flags
#
#   # With Weights & Biases logging
#   WANDB=1 bash scripts/run_swig_ground_groma_qwen_eval.sh 0            # Enable WandB logging
#   WANDB=1 VERBOSE=1 bash scripts/run_swig_ground_groma_qwen_eval.sh 1  # Full debug + WandB
#   WANDB=1 WANDB_PROJECT="my-hoi-eval" WANDB_RUN_NAME="test-run" bash scripts/run_swig_ground_groma_qwen_eval.sh 0
#
...
#   WANDB_PROJECT     W&B project name (default: swig-grounding-groma-qwen)
#   WANDB_RUN_NAME    W&B run name (default: auto-generated)
#
# Output files:
#   {output_dir}/swig_ground_groma_qwen_results_{timestamp}.json                    # Raw predictions
#   {output_dir}/swig_ground_groma_qwen_results_{timestamp}_per_image.json         # Detailed per-image (VERBOSE)
#   {output_dir}/swig_ground_groma_qwen_results_{timestamp}_per_action.json        # Per-action breakdown (VERBOSE)
#   {output_dir}/swig_ground_groma_qwen_results_{timestamp}_metrics.json           # AR metrics
#   {output_dir}/swig_ground_groma_qwen_results_{timestamp}_action_stats.json       # Per-action stats
#   {output_dir}/swig_ground_groma_qwen_evaluation_{timestamp}.log                 # Full log
#   {output_dir}/swig_ground_groma_qwen_results_{timestamp}_visualizations/*.jpg    # Visualizations (VERBOSE)
################################################################################
...
OUTPUT_DIR="${3:-results-redo-groma/swig_ground_groma_qwen}"
...
LOG_FILE="$OUTPUT_DIR/swig_ground_groma_qwen_evaluation_${TIMESTAMP}.log"

# SWIG dataset paths
SWIG_ROOT="../data/swig_hoi"
IMG_PREFIX="${SWIG_ROOT}/images_512"
ANN_FILE="groma_data/benchmarks/swig_ground_test.json"
RESULT_FILE="${OUTPUT_DIR}/swig_ground_groma_qwen_results_${TIMESTAMP}.json"
...
echo "========================================================================"
echo "SWIG-HOI Grounding Evaluation (Groma Qwen3VL)"
echo "========================================================================"
echo "GPU:         $GPU_ID"
...
if [ ! -z "$WANDB" ]; then
    ...
    if [ ! -z "$WANDB_PROJECT" ]; then
        ...
        echo "  WandB project: swig-grounding-groma-qwen (default)"
    fi
...
EVAL_CMD="python3 groma/eval/eval_swig_ground_groma_qwen.py \
