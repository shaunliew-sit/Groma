#!/bin/bash
################################################################################
# HICO-DET Grounding Task Evaluation Script for Groma Qwen3VL
# Evaluates Groma Qwen3VL grounding performance on HICO-DET dataset
...
#
# Usage:
#   bash scripts/run_hico_ground_groma_qwen_eval.sh [GPU] [MODEL] [OUTPUT_DIR]
#
# Examples:
#   # Basic usage
#   bash scripts/run_hico_ground_groma_qwen_eval.sh 0                    # Use GPU 0 with defaults
#   bash scripts/run_hico_ground_groma_qwen_eval.sh 1                    # Use GPU 1 with defaults
#   bash scripts/run_hico_ground_groma_qwen_eval.sh 0,1                  # Use GPUs 0 and 1
#
#   # With optional flags (set as environment variables)
#   VERBOSE=1 bash scripts/run_hico_ground_groma_qwen_eval.sh 0          # Show per-image results + visualizations
#   MAX_IMAGES=10 bash scripts/run_hico_ground_groma_qwen_eval.sh 0      # Test on first 10 images
#   VERBOSE=1 MAX_IMAGES=10 bash scripts/run_hico_ground_groma_qwen_eval.sh 1  # Both flags
#
#   # With Weights & Biases logging
#   WANDB=1 bash scripts/run_hico_ground_groma_qwen_eval.sh 0            # Enable WandB logging
#   WANDB=1 VERBOSE=1 MAX_IMAGES=10 bash scripts/run_hico_ground_groma_qwen_eval.sh 1  # Full debug + WandB
#   WANDB=1 VERBOSE=1 bash scripts/run_hico_ground_groma_qwen_eval.sh 1  # Full debug + WandB
#   WANDB=1 WANDB_PROJECT="my-hoi-eval" WANDB_RUN_NAME="test-run" bash scripts/run_hico_ground_groma_qwen_eval.sh 0
#
...
#   WANDB_PROJECT     W&B project name (default: hico-grounding-groma-qwen)
#   WANDB_RUN_NAME    W&B run name (default: auto-generated)
#
# Output files:
#   {output_dir}/hico_ground_groma_qwen_results_{timestamp}.json                    # Raw predictions
#   {output_dir}/hico_ground_groma_qwen_results_{timestamp}_per_image.json         # Detailed per-image (VERBOSE)
#   {output_dir}/hico_ground_groma_qwen_results_{timestamp}_per_action.json        # Per-action breakdown (VERBOSE)
#   {output_dir}/hico_ground_groma_qwen_results_{timestamp}_metrics.json           # AR metrics
#   {output_dir}/hico_ground_groma_qwen_results_{timestamp}_action_stats.json       # Per-action stats
#   {output_dir}/hico_ground_groma_qwen_evaluation_{timestamp}.log                 # Full log
#   {output_dir}/hico_ground_groma_qwen_results_{timestamp}_visualizations/*.jpg  # Visualizations (VERBOSE)
################################################################################
...
GPU_ID="${1:-0}"
MODEL_PATH="${2:-checkpoints/groma-qwen-stage3-hoi-qwen3vl/checkpoint-2500}"
OUTPUT_DIR="${3:-results-redo-groma/hico_ground_groma_qwen}"
...
LOG_FILE="$OUTPUT_DIR/hico_ground_groma_qwen_evaluation_${TIMESTAMP}.log"

# HICO dataset paths
HICO_ROOT="../data/hico_20160224_det"
IMG_PREFIX="${HICO_ROOT}/images/test2015"
ANN_FILE="groma_data/benchmarks/hico_ground_test.json"
RESULT_FILE="${OUTPUT_DIR}/hico_ground_groma_qwen_results_${TIMESTAMP}.json"
...
echo "========================================================================"
echo "HICO-DET Grounding Evaluation (Groma Qwen3VL)"
echo "========================================================================"
echo "GPU:         $GPU_ID"
...
if [ ! -z "$WANDB" ]; then
    ...
    if [ ! -z "$WANDB_PROJECT" ]; then
        ...
        echo "  WandB project: hico-grounding-groma-qwen (default)"
    fi
...
EVAL_CMD="python3 groma/eval/eval_hico_ground_groma_qwen.py \
...
if [ $? -eq 0 ]; then
    ...
    if [ ! -z "$VERBOSE" ]; then
        ...
        PER_ACTION_FILE="${RESULT_FILE//.json/_action_stats.json}"
