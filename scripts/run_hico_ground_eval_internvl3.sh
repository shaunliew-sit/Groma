#!/bin/bash
################################################################################
# HICO-DET Grounding Task Evaluation Script for InternVL3
# Evaluates InternVL3 grounding performance on HICO-DET dataset
...
#
# Usage:
#   bash scripts/run_hico_ground_eval_internvl3.sh [GPU] [MODEL] [OUTPUT_DIR]
#
# Examples:
#   # Basic usage with different GPUs
#   bash scripts/run_hico_ground_eval_internvl3.sh 0                    # Use GPU 0
#   bash scripts/run_hico_ground_eval_internvl3.sh 1                    # Use GPU 1
#   bash scripts/run_hico_ground_eval_internvl3.sh cuda:0               # Explicit cuda:0
#
#   # With optional flags (set as environment variables)
#   VERBOSE=1 bash scripts/run_hico_ground_eval_internvl3.sh 0          # Show per-image results
#   MAX_IMAGES=10 bash scripts/run_hico_ground_eval_internvl3.sh 0      # Test on first 10 images
#   VERBOSE=1 MAX_IMAGES=10 bash scripts/run_hico_ground_eval_internvl3.sh 0  # Both flags
#
#   # With Weights & Biases logging
#   WANDB=1 bash scripts/run_hico_ground_eval_internvl3.sh 0            # Enable WandB
#   WANDB=1 WANDB_PROJECT="internvl3-hico" bash scripts/run_hico_ground_eval_internvl3.sh 0
#
#   # Use different model size
#   bash scripts/run_hico_ground_eval_internvl3.sh 0 "OpenGVLab/InternVL3-4B"
#
...
#   WANDB_PROJECT     W&B project name (default: hico-grounding-internvl3)
#   WANDB_RUN_NAME    W&B run name (default: auto-generated)
#
# Output files:
#   {output_dir}/hico_ground_internvl3_results_{timestamp}.json          # Raw predictions
#   {output_dir}/hico_ground_internvl3_results_{timestamp}_metrics.json # AR metrics
#   {output_dir}/hico_ground_internvl3_results_{timestamp}_action_stats.json # Per-action
#   {output_dir}/hico_ground_internvl3_evaluation_{timestamp}.log        # Full log
################################################################################
...
OUTPUT_DIR="${3:-results/hico_ground_internvl3}"
...
LOG_FILE="$OUTPUT_DIR/hico_ground_internvl3_evaluation_${TIMESTAMP}.log"
...
RESULT_FILE="${OUTPUT_DIR}/hico_ground_internvl3_results_${TIMESTAMP}.json"
...
echo "========================================================================"
echo "HICO-DET Grounding Evaluation (InternVL3)"
echo "========================================================================"
...
if [ ! -z "$WANDB" ]; then
    ...
    if [ ! -z "$WANDB_PROJECT" ]; then
        ...
        echo "  WandB project: hico-grounding-internvl3 (default)"
    fi
...
EVAL_CMD="python3 groma/eval/eval_hico_ground_internvl3.py \
...
if [ $? -eq 0 ]; then
    ...
    echo "  See results/hico_ground/ for Groma results"
    echo "  See results/hico_ground_qwen3vl/ for Qwen3VL results"
    echo "========================================================================"
else
    echo ""
    echo "========================================================================"
