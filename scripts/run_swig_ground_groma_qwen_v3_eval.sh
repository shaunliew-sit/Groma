#!/bin/bash
################################################################################
# [GROMA-QWEN V3] SWIG-HOI Grounding Task Evaluation Script (JSON Format)
...
#
# Usage:
#   bash scripts/run_swig_ground_groma_qwen_v3_eval.sh [GPU] [MODEL] [BASE_MODEL] [OUTPUT_DIR]
#
# Examples:
#   # Basic usage with V3 model
#   bash scripts/run_swig_ground_groma_qwen_v3_eval.sh 0 checkpoints/groma-qwen-v3-grounding
#
#   # Works with V2 models too (auto-detected)
#   bash scripts/run_swig_ground_groma_qwen_v3_eval.sh 0 checkpoints/groma-qwen-v2-grounding
#
#   # With optional flags (set as environment variables)
#   VERBOSE=1 bash scripts/run_swig_ground_groma_qwen_v3_eval.sh 0
#   MAX_IMAGES=10 bash scripts/run_swig_ground_groma_qwen_v3_eval.sh 0
#   VERBOSE=1 MAX_IMAGES=10 bash scripts/run_swig_ground_groma_qwen_v3_eval.sh 7
#
#   # With Weights & Biases logging
#   WANDB=1 VERBOSE=1 bash scripts/run_swig_ground_groma_qwen_v3_eval.sh 6
#   WANDB=1 WANDB_PROJECT="my-hoi-eval" bash scripts/run_swig_ground_groma_qwen_v3_eval.sh 0
#   WANDB=1 WANDB_PROJECT="my-project" WANDB_RUN_NAME="test-run" bash scripts/run_swig_ground_groma_qwen_v3_eval.sh 0
#
...
#
# Output files:
#   {output_dir}/swig_ground_v3_{timestamp}.json                    # Raw predictions
#   {output_dir}/swig_ground_v3_{timestamp}_per_image.json          # Detailed (VERBOSE)
#   {output_dir}/swig_ground_v3_{timestamp}_metrics.json            # AR metrics
#   {output_dir}/swig_ground_v3_{timestamp}_action_stats.json       # Per-action stats
#   {output_dir}/swig_ground_v3_{timestamp}.log                     # Full log
################################################################################
...
OUTPUT_DIR="${4:-results-groma-qwen/swig_ground_v3}"
...
echo "========================================================================"
echo "[GROMA-QWEN V3] SWIG-HOI Grounding Evaluation (JSON Format)"
echo "========================================================================"
echo "GPU:          $GPU_ID"
