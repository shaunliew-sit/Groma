#!/bin/bash
################################################################################
# HICO-DET Action Referring Task Evaluation Script for InternVL3
...
#
# Usage:
#   bash scripts/run_hico_action_referring_eval_internvl3.sh [GPU] [MODEL] [OUTPUT_DIR]
#
# Examples:
#   # Basic usage with different GPUs
#   bash scripts/run_hico_action_referring_eval_internvl3.sh 0                    # Use GPU 0
#   bash scripts/run_hico_action_referring_eval_internvl3.sh 1                    # Use GPU 1
#   bash scripts/run_hico_action_referring_eval_internvl3.sh cuda:0               # Explicit cuda:0
#
#   # With optional flags (set as environment variables)
#   VERBOSE=1 bash scripts/run_hico_action_referring_eval_internvl3.sh 0          # Show per-triplet results
#   MAX_IMAGES=10 bash scripts/run_hico_action_referring_eval_internvl3.sh 0      # Test on first 10 triplets
#   VERBOSE=1 MAX_IMAGES=10 bash scripts/run_hico_action_referring_eval_internvl3.sh 0  # Both flags
#
#   # With Weights & Biases logging
#   WANDB=1 bash scripts/run_hico_action_referring_eval_internvl3.sh 0            # Enable WandB
#   WANDB=1 WANDB_PROJECT="internvl3-hico-action" bash scripts/run_hico_action_referring_eval_internvl3.sh 0
#
#   # Use different model size
#   bash scripts/run_hico_action_referring_eval_internvl3.sh 0 "OpenGVLab/InternVL3-2B"
#
...
#   WANDB_PROJECT     W&B project name (default: hico-action-referring-internvl3)
#   WANDB_RUN_NAME    W&B run name (default: auto-generated)
#
# Output files:
#   {output_dir}/hico_action_internvl3_results_{timestamp}.json          # Raw predictions
#   {output_dir}/hico_action_internvl3_results_{timestamp}_metrics.json # METEOR/CIDEr scores
#   {output_dir}/hico_action_internvl3_results_{timestamp}_per_triplet.json  # Detailed per-triplet (VERBOSE)
#   {output_dir}/hico_action_internvl3_results_{timestamp}_per_action.json   # Per-action breakdown (VERBOSE)
#   {output_dir}/hico_action_internvl3_evaluation_{timestamp}.log        # Full log
################################################################################
...
OUTPUT_DIR="${3:-results/hico_action_internvl3}"
...
LOG_FILE="$OUTPUT_DIR/hico_action_internvl3_evaluation_${TIMESTAMP}.log"
...
PRED_FILE="${OUTPUT_DIR}/hico_action_internvl3_results_${TIMESTAMP}.json"
...
echo "========================================================================"
echo "HICO-DET Action Referring Evaluation (InternVL3)"
echo "========================================================================"
...
if [ ! -z "$WANDB" ]; then
    ...
    if [ ! -z "$WANDB_PROJECT" ]; then
        ...
        echo "  WandB project: hico-action-referring-internvl3 (default)"
    fi
...
EVAL_CMD="python3 groma/eval/eval_hico_action_referring_internvl3.py \
