#!/bin/bash
################################################################################
# HICO-DET One-Shot HOI Grounding Evaluation Script
# Evaluates one-shot HOI grounding performance on HICO-DET dataset
#
...
#
# Usage:
#   bash scripts/run_hico_hoi_oneshot_eval.sh [GPU] [MODEL] [ANN_FILE] [IMAGES_DIR] [OUTPUT_DIR]
#
# Examples:
#   # Basic usage
#   bash scripts/run_hico_hoi_oneshot_eval.sh 0                    # Use GPU 0 with defaults
#   bash scripts/run_hico_hoi_oneshot_eval.sh 1                    # Use GPU 1 with defaults
#   bash scripts/run_hico_hoi_oneshot_eval.sh 0,1                  # Use GPUs 0 and 1
#
#   # With optional flags (set as environment variables)
#   VERBOSE=1 bash scripts/run_hico_hoi_oneshot_eval.sh 0          # Show per-image results + visualizations
#   MAX_IMAGES=10 bash scripts/run_hico_hoi_oneshot_eval.sh 0      # Test on first 10 images
#   VERBOSE=1 MAX_IMAGES=10 bash scripts/run_hico_hoi_oneshot_eval.sh 0  # Both flags
#
#   # With Weights & Biases logging
#   WANDB=1 bash scripts/run_hico_hoi_oneshot_eval.sh 0            # Enable WandB logging
#   WANDB=1 VERBOSE=1 MAX_IMAGES=10 bash scripts/run_hico_hoi_oneshot_eval.sh 1  # Full debug + WandB
#   WANDB=1 VERBOSE=1 bash scripts/run_hico_hoi_oneshot_eval.sh 1  # Full debug + WandB + full run
#   WANDB=1 WANDB_PROJECT="my-hoi-eval" WANDB_RUN_NAME="test-run" bash scripts/run_hico_hoi_oneshot_eval.sh 0
#
# Output files:
#   - {output_dir}/hico_hoi_oneshot_predictions_{timestamp}.json           # Predictions
#   - {output_dir}/hico_hoi_oneshot_ground_truth_{timestamp}.json          # Ground truth
#   - {output_dir}/hico_hoi_oneshot_predictions_{timestamp}_metrics.json   # Metrics
#   - {output_dir}/hico_hoi_oneshot_evaluation_{timestamp}.log             # Full log
...
LOG_FILE="$OUTPUT_DIR/hico_hoi_oneshot_evaluation_${TIMESTAMP}.log"
RESULT_FILE="$OUTPUT_DIR/hico_hoi_oneshot_predictions_${TIMESTAMP}.json"
...
echo "========================================================================"
echo "HICO-DET One-Shot HOI Grounding Evaluation"
echo "========================================================================"
echo "GPU:         $GPU_ID"
...
EVAL_CMD="python groma/eval/eval_hico_hoi_oneshot.py \
