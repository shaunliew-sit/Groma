#!/bin/bash
################################################################################
# SWIG-HOI Action Referring Task Evaluation Script for InternVL3
...
#
# Note: SWIG actions are in -ing form (e.g., "stapling", "stirring")
#       Supports person-person interactions
#
# Usage:
#   bash scripts/run_swig_action_referring_eval_internvl3.sh [GPU] [MODEL] [OUTPUT_DIR]
#
# Examples:
#   # Basic usage with different GPUs
#   bash scripts/run_swig_action_referring_eval_internvl3.sh 0                    # Use GPU 0
#   bash scripts/run_swig_action_referring_eval_internvl3.sh 1                    # Use GPU 1
#   bash scripts/run_swig_action_referring_eval_internvl3.sh cuda:0               # Explicit cuda:0
#
#   # With optional flags (set as environment variables)
#   VERBOSE=1 bash scripts/run_swig_action_referring_eval_internvl3.sh 0          # Show per-triplet results
#   MAX_IMAGES=10 bash scripts/run_swig_action_referring_eval_internvl3.sh 0      # Test on first 10 triplets
#   VERBOSE=1 MAX_IMAGES=10 bash scripts/run_swig_action_referring_eval_internvl3.sh 0  # Both flags
#
#   # With Weights & Biases logging
#   WANDB=1 bash scripts/run_swig_action_referring_eval_internvl3.sh 0            # Enable WandB
#   WANDB=1 WANDB_PROJECT="internvl3-swig-action" bash scripts/run_swig_action_referring_eval_internvl3.sh 0
#
#   # Use different model size
#   bash scripts/run_swig_action_referring_eval_internvl3.sh 0 "OpenGVLab/InternVL3-2B"
#
...
#   WANDB_PROJECT     W&B project name (default: swig-action-referring-internvl3)
#   WANDB_RUN_NAME    W&B run name (default: auto-generated)
#
# Output files:
#   {output_dir}/swig_action_internvl3_results_{timestamp}.json          # Raw predictions
#   {output_dir}/swig_action_internvl3_results_{timestamp}_metrics.json # METEOR/CIDEr scores
#   {output_dir}/swig_action_internvl3_results_{timestamp}_per_triplet.json  # Detailed per-triplet (VERBOSE)
#   {output_dir}/swig_action_internvl3_results_{timestamp}_per_action.json   # Per-action breakdown (VERBOSE)
#   {output_dir}/swig_action_internvl3_evaluation_{timestamp}.log        # Full log
################################################################################
...
GPU_ID="${1:-0}"
MODEL_NAME="${2:-OpenGVLab/InternVL3-8B}"
OUTPUT_DIR="${3:-results/swig_action_internvl3}"
...
LOG_FILE="$OUTPUT_DIR/swig_action_internvl3_evaluation_${TIMESTAMP}.log"

# SWIG dataset paths
SWIG_ROOT="/Users/shaunliew/Documents/Intent-Identification-Detection/swig_hoi"
IMG_PREFIX="${SWIG_ROOT}/images_512"
ANN_FILE="groma_data/benchmarks/swig_action_referring_test.json"
PRED_FILE="${OUTPUT_DIR}/swig_action_internvl3_results_${TIMESTAMP}.json"
...
echo "========================================================================"
echo "SWIG-HOI Action Referring Evaluation (InternVL3)"
echo "========================================================================"
...
if [ ! -z "$WANDB" ]; then
    ...
    if [ ! -z "$WANDB_PROJECT" ]; then
        ...
        echo "  WandB project: swig-action-referring-internvl3 (default)"
    fi
...
EVAL_CMD="python3 groma/eval/eval_swig_action_referring_internvl3.py \
...
if [ $? -eq 0 ]; then
    ...
    echo "  See results/swig_action_referring/ for Groma results"
    echo "========================================================================"
else
    echo ""
    echo "========================================================================"
