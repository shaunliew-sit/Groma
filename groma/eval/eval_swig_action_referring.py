"""
SWIG-HOI Action Referring Evaluation Script
...
Usage:
    python groma/eval/eval_swig_action_referring.py \
        ...
        --ann-file groma_data/swig_action_referring_test.json \
...
from groma.data.datasets.swig_action_referring import SWIGActionReferringTest, collate_fn
...
def eval_model(args):
    ...
    print("=" * 80)
    print("SWIG-HOI Action Referring Evaluation (METEOR & CIDEr)")
    print("=" * 80)
    ...
    if args.wandb:
        if not WANDB_AVAILABLE:
            ...
            wandb.init(
                ...
                name=args.wandb_run_name or f"swig-action-{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                config={
    ...
    dataset = SWIGActionReferringTest(
    ...
    for batch in tqdm(dataloader, desc="Generating action predictions", unit="triplet"):
        ...
        person_bbox = batch['person_bbox'].cuda().squeeze(0)  # Remove batch dim: (1, 4) -> (4,)
        object_bbox = batch['object_bbox'].cuda().squeeze(0)  # Remove batch dim: (1, 4) -> (4,)
        ...
        refer_boxes = torch.stack([person_bbox, object_bbox], dim=0)  # (2, 4)
        ...
        with torch.inference_mode():
            outputs = model.generate(
                input_ids,
                images=image,
                refer_boxes=[refer_boxes],  # Pass as list containing (2, 4) tensor
                ground_boxes=None,  # Explicitly pass None for action referring task
                use_cache=True,
                do_sample=False,
    ...
    # Print results
    print(f"\n{'='*80}")
    print("SWIG-HOI Action Referring Evaluation Results")
    print(f"{'='*80}")
    ...
    metrics = {
        ...
        "model": args.model_name,
        "dataset": "SWIG-HOI Action Referring"
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SWIG-HOI Action Referring Evaluation with METEOR/CIDEr",
...
Examples:
    ...
    python groma/eval/eval_swig_action_referring.py \\
        ...
        --metrics-file results/metrics.json \\
        --wandb \\
        --wandb-project swig-action-eval \\
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to SWIG-HOI images directory (e.g., images_512)"
    )
    parser.add_argument(
        "--ann-file",
        type=str,
    ...
    parser.add_argument(
        "--wandb-project",
        type=str,
        default="swig-action-referring",
        help="W&B project name (default: swig-action-referring)"
    )
    parser.add_argument(
        "--wandb-run-name",
        type=str,
