"""
SWIG-HOI Grounding Evaluation Script (Multi-Pair Format)

Evaluates grounding performance on SWIG-HOI dataset with multi-pair support.
...
def parse_grounding_response(response_text, pred_boxes, img_shape):
    ...
    for line in lines:
        ...
        # Pattern: <p>person</p><roi><r{idx}></roi><p>object/person</p><roi><r{idx}></roi>
        region_pattern = r'<r(\d+)>'
        matches = re.findall(region_pattern, line)

        if len(matches) >= 2:
            # First region is person, second is object (or another person)
...
def visualize_grounding_results(img_path, all_pred_boxes, pred_pairs, gt_pairs, matches,
                                  ...
                                  generated_text, action, object_category, is_person_person=False):
    """
    Visualize grounding results with 3-panel layout.
    Supports person-person interactions.

    Left: All proposed regions from detector
    Middle: Predicted person-object/person pairs with IoU analysis
    Right: Ground truth person-object/person pairs
    """
    # Load original image
    ...
    # Label for object box (person or object)
    object_label = "Person" if is_person_person else object_category
    ...
    for match in matches:
        ...
        label_o = f"{object_label}(R{pred_pair['object_idx']})\nIoU:{object_iou:.2f}"
        try:
    ...
    for gt_idx, gt_pair in enumerate(gt_pairs):
        ...
        label_o = f"GT_{object_label}_{gt_idx}"
        try:
    ...
    # Add titles
    interaction_type = "[P-P]" if is_person_person else "[P-O]"
    draw_left.text((10, 10), f"All Regions (N={len(all_pred_boxes)}) {interaction_type}", fill='white', font=font)
    draw_left.rectangle([8, 8, 350, 28], outline='black', width=2)

    draw_middle.text((10, 10), f"Predicted Pairs {interaction_type}", fill='white', font=font)
    draw_middle.rectangle([8, 8, 250, 28], outline='black', width=2)

    draw_right.text((10, 10), f"Ground Truth Pairs {interaction_type}", fill='white', font=font)
    draw_right.rectangle([8, 8, 280, 28], outline='black', width=2)
    ...
    # Action-object query with type indicator
    query_text = f"Query: person {action} {object_category} {interaction_type}"
    legend_draw.text((10, y_offset), query_text, fill='black', font=font)
    y_offset += 20
...
def eval_model(args):
    ...
    if use_wandb:
        wandb.init(
            ...
            name=args.wandb_run_name or f"swig_ground_{model_short_name}_{timestamp}",
            config={
            ...
            tags=["swig", "grounding", "multi-pair"]
        )
    ...
    # Separate metrics for person-person vs person-object
    results_person_person = {
        iou_thr: {'tp': 0, 'fp': 0, 'fn': 0}
        for iou_thr in iou_thresholds_ar
    }
    results_person_object = {
        iou_thr: {'tp': 0, 'fp': 0, 'fn': 0}
        for iou_thr in iou_thresholds_ar
    }
    ...
    for idx, batch in enumerate(tqdm(dataloader, disable=show_verbose)):
        ...
        is_person_person = batch['is_person_person']
        ...
        if show_verbose:
            ...
            print(f"  Person-person: {is_person_person}")
        ...
        action_stats[action]['is_person_person'] = is_person_person
        ...
        for iou_thr in iou_thresholds:
            ...
            # Update person-person vs person-object metrics
            if is_person_person:
                results_person_person[iou_thr]['tp'] += len(matches)
                results_person_person[iou_thr]['fp'] += len(unmatched_preds)
                results_person_person[iou_thr]['fn'] += len(unmatched_gts)
            else:
                results_person_object[iou_thr]['tp'] += len(matches)
                results_person_object[iou_thr]['fp'] += len(unmatched_preds)
                results_person_object[iou_thr]['fn'] += len(unmatched_gts)
        ...
        if show_verbose and viz_output_dir is not None:
            ...
            try:
                ...
                interaction_label = "P-P" if is_person_person else "P-O"
                print(f"  💾 Visualization saved [{interaction_label}]: {viz_path}")
                    ...
                    try:
                        wandb.log({
                            f"visualizations/{file_name}": wandb.Image(
                                viz_path,
                                caption=f"Sample {idx+1}: {action} {object_category} [{interaction_label}] | {len(matches_05)}/{len(gt_pairs)} matched"
                            )
                        })
    ...
    print("\n" + "=" * 80)
    print("SWIG-HOI Grounding Evaluation Results (Multi-Pair Format)")
    print("=" * 80)
    ...
    # Compute person-person specific AR
    recalls_pp = []
    for iou_thr in iou_thresholds_ar:
        tp = results_person_person[iou_thr]['tp']
        fn = results_person_person[iou_thr]['fn']
        recall = tp / (tp + fn) if (tp + fn) > 0 else 0.0
        recalls_pp.append(recall)
    ar_pp = float(np.mean(recalls_pp)) if recalls_pp else 0.0
    ar_pp_50 = recalls_pp[0] if len(recalls_pp) > 0 else 0.0

    # Compute person-object specific AR
    recalls_po = []
    for iou_thr in iou_thresholds_ar:
        tp = results_person_object[iou_thr]['tp']
        fn = results_person_object[iou_thr]['fn']
        recall = tp / (tp + fn) if (tp + fn) > 0 else 0.0
        recalls_po.append(recall)
    ar_po = float(np.mean(recalls_po)) if recalls_po else 0.0
    ar_po_50 = recalls_po[0] if len(recalls_po) > 0 else 0.0

    # Add person-person and person-object specific metrics
    metrics['PP_AR'] = ar_pp
    metrics['PP_AR@0.5'] = ar_pp_50
    metrics['PO_AR'] = ar_po
    metrics['PO_AR@0.5'] = ar_po_50
    ...
    print(f"{'PP_AR':<12} {metrics['PP_AR']*100:>9.1f}%  {'Person-Person Average Recall':<50}")
    print(f"{'PP_AR@0.5':<12} {metrics['PP_AR@0.5']*100:>9.1f}%  {'Person-Person AR @ IoU=0.50':<50}")
    print(f"\n### Person-Object Specific ###")
    print(f"{'PO_AR':<12} {metrics['PO_AR']*100:>9.1f}%  {'Person-Object Average Recall':<50}")
    print(f"{'PO_AR@0.5':<12} {metrics['PO_AR@0.5']*100:>9.1f}%  {'Person-Object AR @ IoU=0.50':<50}")
    print("-" * 80)
    ...
    if use_wandb:
        ...
        wandb.log({
            ...
            'person_person_samples': sum(1 for s in per_sample_results if s['is_person_person']),
            'person_object_samples': sum(1 for s in per_sample_results if not s['is_person_person']),
        })
    ...
    print("=" * 80)
    print(f"{'Action':<20} {'Type':>6} {'Samples':>8} {'GT':>6} {'Pred':>6} {'Match':>6} {'R@0.5':>8}")
    print("-" * 80)
    ...
    for action, stats in sorted_actions[:20]:
        ...
        action_type = "P-P" if stats['is_person_person'] else "P-O"
        print(f"{action:<20} {action_type:>6} {stats['total_samples']:>8} "
              f"{stats['total_gt_pairs']:>6} {stats['total_pred_pairs']:>6} "
              f"{stats['matched_pairs_05']:>6} {recall_05 * 100:>7.1f}%")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SWIG-HOI Grounding Evaluation (Multi-Pair)")
    parser.add_argument("--model-name", type=str, required=True,
                        help="Path to Groma model checkpoint")
    parser.add_argument("--ann-file", type=str, required=True,
                        help="Path to SWIG grounding annotation file (e.g., swig_ground_test.json)")
    ...
    parser.add_argument("--wandb-project", type=str, default="groma-swig-ground",
