...
def eval_model(args):
    ...
    if not show_verbose:  # Only show samples if we didn't show all
        ...
        for i, result in enumerate(successful[:5]):
            ...
            # Get sample generated text from first category
            if result['generated_text_per_category']:
                first_cat = list(result['generated_text_per_category'].keys())[0]
                sample_text = result['generated_text_per_category'][first_cat]
                print(f"      Sample output ({first_cat}): {sample_text[:80]}...")
    ...
    # Modify maxDets to ensure ALL boxes are evaluated (not just top 100)
    # Standard COCO uses [1, 10, 100], we'll use [1, 10, 500] to be safe
    coco_eval.params.maxDets = [1, 10, 500]
    print(f"Using maxDets = {coco_eval.params.maxDets} (evaluating up to 500 boxes per image)")
    ...
    # Note: stats[8-11] use the highest maxDets value (now 500 instead of default 100)
    metrics = {
        ...
        'AR': coco_eval.stats[8],        # AR @ maxDets=500
        'ARs': coco_eval.stats[9],       # AR small @ maxDets=500
        'ARm': coco_eval.stats[10],      # AR medium @ maxDets=500
        'ARl': coco_eval.stats[11]       # AR large @ maxDets=500
    }

    # Compute LVIS-Ground style metrics (AR at specific IoU thresholds)
    # AR@0.5: Average Recall at IoU=0.5
    # AR@0.75: Average Recall at IoU=0.75
    def compute_ar_at_iou(coco_eval, iou_thr):
        """Compute Average Recall at specific IoU threshold"""
        # Find the index for the specific IoU threshold
        ...
        # Get recall at this IoU threshold, across all categories, all areas, max_dets=500
        # Shape after selection: [K] (categories only)
        recall_at_iou = recalls[iou_idx, :, 0, 2]  # 0: all area, 2: maxDets=500 (highest maxDets)
    ...
    def compute_ar_at_iou_size(coco_eval, iou_thr, size_idx):
        """Compute Average Recall at specific IoU threshold and size"""
        # size_idx: 1=small, 2=medium, 3=large
        iou_thresholds = coco_eval.params.iouThrs
        if iou_thr in iou_thresholds:
            iou_idx = list(iou_thresholds).index(iou_thr)
        else:
        ...
        recall_at_iou = recalls[iou_idx, :, size_idx, 2]
        valid_recalls = recall_at_iou[recall_at_iou > -1]
        ar = valid_recalls.mean() if len(valid_recalls) > 0 else 0.0

        return float(ar)

    # Compute LVIS-Ground style AR metrics
    ar_05 = compute_ar_at_iou(coco_eval, 0.5)
    ar_075 = compute_ar_at_iou(coco_eval, 0.75)

    # Add to metrics dictionary
    metrics['AR@0.5'] = ar_05
    metrics['AR@0.75'] = ar_075
    ...
    print("-" * 70)
    print(f"{'AR':<12} {metrics['AR']*100:>7.1f}%  {'Average Recall @ IoU=0.50:0.95, maxDets=500':<40}")
    print(f"{'AR@0.5':<12} {metrics['AR@0.5']*100:>7.1f}%  {'Average Recall @ IoU=0.50, maxDets=500':<40}")
    print(f"{'AR@0.75':<12} {metrics['AR@0.75']*100:>7.1f}%  {'Average Recall @ IoU=0.75, maxDets=500':<40}")
    print(f"{'AR@s':<12} {metrics['ARs']*100:>7.1f}%  {'Average Recall for small objects (<32²px)':<40}")
