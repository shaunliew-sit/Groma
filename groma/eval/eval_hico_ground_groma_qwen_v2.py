"""
HICO-DET / SWIG-HOI Grounding Evaluation Script for Groma Qwen V2 (JSON Format)

Evaluates Groma Qwen V2 grounding performance using JSON bbox format aligned with Qwen3-VL.
...
def parse_groma_qwen_v2_grounding_response(response_text, img_shape):
    """
    Parse Groma Qwen V2 grounding response to extract person-object pairs.
...
def run_groma_qwen_v2_inference(model, processor, image_path, action, object_category, gpu_id=0):
    """
    Run Groma Qwen V2 inference for grounding task.

    Args:
        model: GromaQwenModel V2
        processor: Processor
...
def eval_model(args):
    ...
    print("=" * 80)
    print(f"[GROMA-QWEN V2] {dataset_name} Grounding Evaluation (Native Architecture)")
    print("=" * 80)
    ...
    if use_wandb:
        ...
        try:
            ...
            wandb.init(
                ...
                name=args.wandb_run_name or f"groma_qwen_v2_ground_{dataset_name.lower()}_{timestamp}",
                config={
                ...
                tags=["groma-qwen-v2", dataset_name.lower(), "grounding"]
            )
    ...
    for idx, sample in enumerate(tqdm(dataset_samples, disable=show_verbose)):
        ...
        # Parse predicted pairs using V2 native format
        pred_pairs = parse_groma_qwen_v2_grounding_response(output_text, img_shape)
    ...
    print("\n" + "=" * 80)
    print(f"[GROMA-QWEN V2] {dataset_name} Grounding Evaluation Results")
    print("=" * 80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HICO-DET/SWIG Grounding Evaluation with Groma Qwen V2")
    parser.add_argument("--model-name", type=str, required=True,
                        help="Path to Groma Qwen V2 model checkpoint")
    ...
    parser.add_argument("--wandb-project", type=str, default="groma-qwen-v2-grounding",
