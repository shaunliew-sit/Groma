"""
SWIG-HOI Action Referring Evaluation Script for Groma Qwen3VL
...
Usage:
    python groma/eval/eval_swig_action_referring_groma_qwen.py \
        --model-name checkpoints/groma-qwen-stage3-hoi-qwen3vl/checkpoint-2500 \
        --img-prefix /path/to/swig/images_512 \
        --ann-file groma_data/benchmarks/swig_action_referring_test.json \
...
def build_referring_prompt(query_template=None):
    """
    Build prompt for referring task with <roi> <refer_box> </roi> <refer_feat> tokens.
    
    Args:
        query_template: Optional custom query template. If None, uses default.
    
    Returns:
        Prompt text with refer_box tokens
    """
    if query_template:
        # Replace {} placeholders with refer_box tokens
        subject_ref = DEFAULT_TOKENS['bor'] + DEFAULT_TOKENS['rbox'] + DEFAULT_TOKENS['eor'] + DEFAULT_TOKENS['rfeat']
        object_ref = DEFAULT_TOKENS['bor'] + DEFAULT_TOKENS['rbox'] + DEFAULT_TOKENS['eor'] + DEFAULT_TOKENS['rfeat']
        prompt_text = query_template.format(subject_ref, object_ref)
    else:
        # Default prompt from dataset format
        prompt_text = (
            f"What is {DEFAULT_TOKENS['bor']} {DEFAULT_TOKENS['rbox']} {DEFAULT_TOKENS['eor']} {DEFAULT_TOKENS['rfeat']} "
            f"doing with {DEFAULT_TOKENS['bor']} {DEFAULT_TOKENS['rbox']} {DEFAULT_TOKENS['eor']} {DEFAULT_TOKENS['rfeat']}? "
...
def run_groma_qwen_referring_inference(model, processor, image_path, person_bbox, object_bbox, gpu_id=0):
    """
    ...
    # Build prompt
    prompt_text = build_referring_prompt()
    ...
    region_images = prepare_region_images(
        image, 
        [person_bbox, object_bbox], target_size=224  # DINOv2 input size
    ).to(model_device)
...
def eval_model(args):
    ...
    print("=" * 80)
    print("SWIG-HOI Action Referring Evaluation (Groma Qwen3VL)")
    print("=" * 80)
    ...
    if use_wandb:
        ...
        try:
            ...
            wandb.init(
                ...
                name=args.wandb_run_name or f"swig_action_groma_qwen_{timestamp}",
                config={
                ...
                tags=["swig", "action-referring", "groma-qwen"]
            )
    ...
    for idx, sample in enumerate(tqdm(dataset_samples, desc="Generating action predictions", unit="triplet")):
        ...
        output_text, image = run_groma_qwen_referring_inference(
            model, processor, img_path, person_bbox, object_bbox, gpu_id
        )
    ...
    print("=" * 80)
    print("Note: SWIG actions are in -ing form (e.g., 'stirring', 'stapling')")
    print("      Dataset includes person-person interactions")
    print("=" * 80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SWIG-HOI Action Referring Evaluation with Groma Qwen3VL")
    ...
    parser.add_argument("--wandb-project", type=str, default="swig-action-referring-groma-qwen",
