"""
[GROMA-QWEN V2] HICO-DET Action Referring Evaluation Script (Native Architecture)
...
- No custom tokens (<region>, <refer_feat>, etc.)
- No region_images preprocessing (model extracts from Qwen3VL vision encoder)
...
def load_groma_qwen_model_v2(model_name, base_model_name=None, gpu_id=0):
    """Load GromaQwenModel V2 and processor."""
    model_name = os.path.expanduser(model_name)
    
    print("=" * 80)
    print(f"[GROMA-QWEN V2] Loading model from {model_name}")
    print("=" * 80)
...
def build_referring_prompt_native(subject_bbox_qwen, object_bbox_qwen):
...
def run_groma_qwen_v2_inference(model, processor, image_path, person_bbox, object_bbox):
    """Run Groma Qwen V2 inference for referring task."""
    ...
    system_message, user_query = build_referring_prompt_native(person_bbox_qwen, object_bbox_qwen)
...
def eval_model(args):
    ...
    print("=" * 80)
    print("[GROMA-QWEN V2] HICO-DET Action Referring Evaluation (Native Architecture)")
    print("=" * 80)
    ...
    if use_wandb:
        ...
        try:
            ...
            wandb.init(
                ...
                name=args.wandb_run_name or f"groma_qwen_v2_{dataset_name}_{timestamp}",
                config={
                ...
                tags=["groma-qwen-v2", dataset_name, "action-referring"]
            )
    ...
    # Load model
    model, processor, tokenizer = load_groma_qwen_model_v2(
        args.model_name,
    ...
    if args.verbose:
        ...
        viz_dir = os.path.join(pred_dir, f"visualizations_v2_{timestamp}")
    ...
    for idx, sample in enumerate(tqdm(dataset_samples, desc="Generating predictions")):
        ...
        output_text, image = run_groma_qwen_v2_inference(
            model, processor, img_path, person_bbox, object_bbox
        )
    ...
    coco_gt_data = {
        "info": {"description": "HICO-DET Action Referring V2"},
        "licenses": [],
        "images": [],
        "annotations": []
    }
    ...
    if use_wandb:
        ...
        wandb.log({
            'total_triplets': len(coco_pred_results),
            'exact_matches': sum(1 for r in per_triplet_results if r.get('exact_match', False)),
        })
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HICO-DET Action Referring Evaluation (Groma Qwen V2)")
    ...
    parser.add_argument("--img-prefix", type=str, required=True, help="HICO images directory")
    ...
    parser.add_argument("--wandb-project", type=str, default="groma-qwen-v2-eval",
