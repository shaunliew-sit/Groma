"""
HICO-DET to Action Referring Instruction Converter

Converts HICO-DET annotations to Groma's action referring format with action+object output.

Output format: "action object" (e.g., "riding motorcycle", "holding bottle")

Usage:
    python groma/data/annotation_converters/hico_to_action_referring.py \
        --anno-file /path/to/trainval_hico_ann.json \
        --output-file groma_data/benchmarks/hico_action_referring_train.json
"""
...
def convert_single_image_hico_action_referring(
    ann: Dict,
    action_id2name: Dict,
    object_id2name: Dict
) -> List[Dict]:
    """
    Convert single HICO image to action referring format.
    Output: "action object" (e.g., "riding motorcycle")
    ...
    for triplet in hoi_triplets:
        ...
        # Response: "action object" (e.g., "riding motorcycle")
...
def convert_hico_to_action_referring(
    anno_file: str,
    output_file: str,
    max_images: Optional[int] = None
) -> None:
    """
    Convert HICO-DET to action referring instruction format.
    ...
    print(f"\n{'='*80}")
    print(f"HICO-DET to Action Referring Converter")
    print(f"Output format: 'action object' (e.g., 'riding motorcycle')")
    print(f"{'='*80}\n")
    ...
    for idx, ann in enumerate(annotations):
        ...
        instructions = convert_single_image_hico_action_referring(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Convert HICO-DET to action referring format (action+object)',
    ...
    convert_hico_to_action_referring(
