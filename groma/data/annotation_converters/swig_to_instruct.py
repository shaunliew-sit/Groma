#!/usr/bin/env python3
"""
SWIG-HOI to Groma Grounding + Referring Instruction Converter

Converts SWIG-HOI annotations to Groma's instruction format for:
...
def convert_single_image_swig_both_tasks(
    ann: Dict,
    action_id2name: Dict,
    object_id2name: Dict,
    ...
    """
    Convert single SWIG image to BOTH grounding and referring formats.
...
def convert_swig_to_grounding_and_referring(
    anno_file: str,
    output_file: str,
    ...
    """
    Convert SWIG-HOI to BOTH grounding and referring instruction formats.
    ...
    print(f"\n{'='*80}")
    print(f"SWIG-HOI to Groma Grounding + Referring Converter")
    print(f"{'='*80}\n")
    ...
    for idx, ann in enumerate(annotations):
        ...
        for var_idx in range(num_grounding_variations):
            instructions = convert_single_image_swig_both_tasks(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Convert SWIG-HOI to Groma grounding + referring format',
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to SWIG annotation file (swig_train_1000.json)'
    )
    parser.add_argument(
        '--output-file',
        type=str,
    ...
    convert_swig_to_grounding_and_referring(
